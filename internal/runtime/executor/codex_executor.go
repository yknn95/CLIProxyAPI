...
func (e *CodexExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	for attempt := 0; attempt < attempts; attempt++ {
		attemptIdentityState := identityState
		httpResp, errRequest := e.doCodexRequest(ctx, auth, reporter, func() (*http.Request, error) {
			httpReq, _, identityStateBuild, errBuild := e.cacheHelper(ctx, from, url, auth, req, originalPayloadSource, body)
			if errBuild != nil {
				return nil, errBuild
			}
			attemptIdentityState = identityStateBuild
			applyCodexHeaders(httpReq, auth, apiKey, true, e.cfg)
			applyCodexIdentityConfuseHeaders(httpReq.Header, &attemptIdentityState)
			return httpReq, nil
		})
		if errRequest != nil {
		...
		respAttempt, errAttempt, retryable := e.finishCodexNonStreamResponse(ctx, reporter, httpResp, req.Model, from, to, originalPayload, body, attemptIdentityState, replayScope)
		if errAttempt == nil {
...
func (e *CodexExecutor) executeCompact(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	httpResp, err := e.doCodexRequest(ctx, auth, reporter, func() (*http.Request, error) {
		httpReq, _, identityStateBuild, errBuild := e.cacheHelper(ctx, from, url, auth, req, originalPayloadSource, body)
		if errBuild != nil {
			return nil, errBuild
		}
		identityState = identityStateBuild
		applyCodexHeaders(httpReq, auth, apiKey, false, e.cfg)
		applyCodexIdentityConfuseHeaders(httpReq.Header, &identityState)
		return httpReq, nil
	})
...
func (e *CodexExecutor) doCodexRequest(ctx context.Context, auth *cliproxyauth.Auth, reporter *helps.UsageReporter, buildRequest func() (*http.Request, error)) (*http.Response, error) {
...
func (e *CodexExecutor) finishCodexNonStreamResponse(ctx context.Context, reporter *helps.UsageReporter, httpResp *http.Response, requestModel string, from sdktranslator.Format, to sdktranslator.Format, originalPayload []byte, body []byte, identityState codexIdentityConfuseState, replayScope codexReasoningReplayScope) (cliproxyexecutor.Response, error, bool) {
	...
	for scanner.Scan() {
		...
		out := sdktranslator.TranslateNonStream(ctx, to, from, requestModel, originalPayload, body, clientCompletedData, &param)
		resp = cliproxyexecutor.Response{Payload: out, Headers: httpResp.Header.Clone()}
